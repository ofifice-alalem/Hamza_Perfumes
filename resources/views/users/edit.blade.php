...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="mb-1"><i class="fas fa-user-edit me-2 text-primary"></i>تعديل مستخدم</h2>
        <p class="text-muted mb-0">تعديل بيانات المستخدم</p>
    </div>
    ...
    <div class="col-lg-8">
        <div class="card-modern">
            <div class="card-header" style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; border-radius: 15px 15px 0 0; padding: 20px;">
                <h5 class="mb-0 fw-bold"><i class="fas fa-user-edit me-2"></i>تعديل بيانات {{ $user->username }}</h5>
            </div>
                ...
                <form action="{{ route('users.update', $user) }}" method="POST">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('username') is-invalid @enderror" id="username" name="username" value="{{ old('username', $user->username) }}" required style="border-radius: 15px; padding: 12px 20px; border: 2px solid #e9ecef; transition: all 0.3s;">
                            @error('username')
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required style="border-radius: 15px; padding: 12px 20px; border: 2px solid #e9ecef; transition: all 0.3s;">
                                ...
                                <option value="super-admin" {{ old('role', $user->role) === 'super-admin' ? 'selected' : '' }}>مدير عام</option>
                                <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>مدير</option>
                                <option value="saler" {{ old('role', $user->role) === 'saler' ? 'selected' : '' }}>بائع</option>
                            </select>
                            @error('role')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-12">
                            <label for="password" class="form-label fw-semibold">
                                <i class="fas fa-lock me-2 text-danger"></i>كلمة المرور الجديدة (اختياري)
                            </label>
                            <input type="password" class="form-control @error('password') is-invalid @enderror" id="password" name="password" style="border-radius: 15px; padding: 12px 20px; border: 2px solid #e9ecef; transition: all 0.3s;">
                    ...
                    <div class="d-flex gap-3 mt-5">
                        <button type="submit" class="btn btn-warning btn-modern flex-fill" style="border-radius: 15px; padding: 12px; font-weight: 600; background: linear-gradient(135deg, #ffc107, #ff8c00);">
